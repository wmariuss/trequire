                ...
                for bucket in buckets_list_add:
                    if bucket not in s3_buckets_list:
                        ...
                        if "created" in create_bucket:
                            click.echo(
                                colored("[{}] s3 bucket created".format(bucket), "green")
                            )
                    else:
                        click.echo(
                            colored(
                                "[{}] bucket already exists, but it will be updated".format(
                                    bucket
                                ),
                                "yellow",
                            )
                ...
                for table in tables_list_add:
                    if table not in dynamodb_tables_list:
                        ...
                        if "created" in create_dynamodb_table:
                            click.echo(
                                colored(
                                    "[{}] dynamodb table created".format(table), "green"
                                )
                            )
                    else:
                        click.echo(
                            colored(
                                "[{}] dynamodb table already exists".format(table),
                                "yellow",
                            )
                        )
            ...
            if iam_user:
                if iam_user not in users_list:
                    ...
                    if "success" in add_user.lower():
                        click.echo(
                            colored("[{}] IAM user created".format(iam_user), "green")
                        )
                        click.echo(
                            colored(
                                "Generating access keys for {} user...".format(iam_user),
                                "green",
                            )
                        )
                        click.echo(
                            colored(manage.enable_access_key(name=iam_user), "green")
                        )
                else:
                ...
                for bucket in buckets_list_remove:
                    if bucket in s3_buckets_list:
                        ...
                        if "removed" in delete_bucket.lower():
                            click.echo(
                                colored("[{}] s3 bucket removed".format(bucket), "green")
                            )
                    else:
                        click.echo(
                            colored(
                                "[{}] s3 bucket is already removed or not exists".format(
                                    bucket
                                ),
                                "yellow",
                            )
                ...
                for table in tables_list_remove:
                    if table in dynamodb_tables_list:
                        ...
                        if "removed" in delete_table.lower():
                            click.echo(
                                colored(
                                    "[{}] dynamodb table removed".format(table), "green"
                                )
                            )
                    else:
                        click.echo(
                            colored(
                                "[{}] dynamodb table is already removed or not exists".format(
                                    table
                                ),
                                "yellow",
                            )
            ...
            if iam_user:
                if iam_user in users_list:
                    ...
                    if "success" in remove_user.lower():
                        click.echo(
                            colored("[{}] IAM user removed".format(iam_user), "green")
                        )
                else:
                    click.echo(
                        colored("[{}] IAM user is not exists".format(iam_user), "yellow")
                    )
